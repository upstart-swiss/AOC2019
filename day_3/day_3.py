class Wire:
    ...
    def draw_line(self, line):
        direction = self.directions[line[0]]
        distance = int(line[1:])
        for i in range(distance):
            self.current_step += 1
            self.move_position(direction)
    
    def move_position(self, adder):
        x = self.current_position[0] + adder[0]
        y = self.current_position[1] + adder[1]
        new_position = (x,y)
        self.current_position = new_position
        self.gridpoints.append(self.current_position)
        if new_position not in self.gridpoint_distance:
            self.gridpoint_distance[new_position] = self.current_step
        return

    def __repr__(self):
        return str(self.gridpoints)


def find_intersection(wire1, wire2):
    commonalities = []
    commonalities = set(wire1.gridpoints).intersection(set(wire2.gridpoints))
    commonalities.remove((0,0))
    if len(commonalities) == 0:
        raise Exception("NO DISTANCE")
    return commonalities

def find_closest_point(wire1, wire2):
    commonalities = find_intersection(wire1, wire2)
    closest_distance = -1
...
def find_closest_signal(wire1, wire2):
    commonalities = find_intersection(wire1, wire2)
    closest_distance = -1
    for point in commonalities:
        distance = wire1.gridpoint_distance[point] + wire2.gridpoint_distance[point]
        if distance < closest_distance or closest_distance == -1:
            closest_distance = distance
    return closest_distance
